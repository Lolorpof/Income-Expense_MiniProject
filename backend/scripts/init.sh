...
INIT_FLAG_FILE="/app/.init/initialized"
...
if [ ! -f "$INIT_FLAG_FILE" ]; then
    ...
    until PGPASSWORD=${POSTGRES_SUPER_PASSWORD} psql -h database -U ${POSTGRES_SUPER_USER} -d ${POSTGRES_DB} -c '\q'; do
      echo "Postgres is unavailable - sleeping"
      sleep 1
    done

    echo "Setting up database user permissions..."
    PGPASSWORD=${POSTGRES_SUPER_PASSWORD} psql -h database -U ${POSTGRES_SUPER_USER} -d ${POSTGRES_DB} << EOF
    DO \$\$
    BEGIN
